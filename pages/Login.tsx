        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:flex lg:items-center">
          <div className="w-full py-8 md:py-12 lg:py-12">
            ...
            <div className="lg:hidden flex flex-col gap-8 md:gap-10 max-w-5xl mx-auto">
              ...
              {showFAQ ? (
                <div className="w-full overflow-y-auto max-h-[55vh] pr-1 rounded-xl">
                  <FAQ />
                </div>
              ) : (
                  ...
                  <div className="relative overflow-hidden rounded-xl shadow-2xl">
                    {images.map((img, index) => (
                      <img
                        ...
                        width="400"
                        height="300"
                        className={`rounded-xl border-2 border-gray-700 w-full ${index === currentImage ? "relative" : "absolute inset-0"}`}
                        style={{
                  ...
                  <div className="flex justify-center items-center gap-3 mt-6">
                    {images.map((_, index) => (
                      <button
                        ...
                        className={`w-3 h-3 md:w-3.5 md:h-3.5 rounded-full transition-all duration-500 ${index === currentImage ? "bg-brand-orange shadow-lg shadow-brand-orange/50 scale-125" : "bg-gray-600 hover:bg-gray-500"}`}
            ...
            <div className="hidden lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center">
              ...
              {showFAQ ? (
                <div className="overflow-y-auto max-h-[60vh] pr-2 rounded-lg">
                  <FAQ />
                </div>
              ) : (
                <>
                  <div className="relative overflow-hidden rounded-lg">
                    {images.map((img, index) => (
                      <img
                        ...
                        width="600"
                        height="400"
                        className={`rounded-lg border border-gray-800 shadow-2xl w-full ${index === currentImage ? "relative" : "absolute inset-0"}`}
                        style={{
                          opacity: index === currentImage ? 1 : 0,
                          transform: index === currentImage ? "scale(1)" : "scale(0.95)",
                          transition: "opacity 0.8s ease-in-out, transform 0.8s ease-in-out",
                  ...
                  <div className="flex justify-center items-center gap-2 mt-6">
                    {images.map((_, index) => (
                      <button
                        key={index}
                        onClick={() => setCurrentImage(index)}
                        className={`w-2.5 h-2.5 rounded-full transition-all duration-500 ${index === currentImage ? "bg-brand-orange shadow-lg shadow-brand-orange/50" : "bg-gray-600 hover:bg-gray-500"}`}
                        style={{ transition: "all 0.6s cubic-bezier(0.68, -0.55, 0.265, 1.55)" }}
                      />
                    ))}
                  </div>
