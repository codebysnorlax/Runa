...
const DashboardSkeleton: React.FC = () => (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-1 flex flex-col items-center justify-center space-y-4 p-6">
        ...
        <Card className="lg:col-span-3 space-y-4 p-6">
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2">
                     <h2 className="text-lg font-semibold text-white mb-2">Latest AI Insight</h2>
                     {latestInsight ? (
                        <div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 {personalRecords && (
                    <Card className="animate-slide-in-right">
                        <h2 className="text-lg font-semibold text-white mb-4 flex items-center"><Trophy className="w-5 h-5 mr-2 text-yellow-400"/> Personal Records</h2>
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="text-gray-300 flex items-center"><Route className="w-4 h-4 mr-2"/> Longest Distance</span>
                                <span className="font-bold text-white">{(personalRecords.longestDistance / 1000).toFixed(2)} km</span>
                            </div>
                             <div className="flex items-center justify-between">
                                <span className="text-gray-300 flex items-center"><Clock className="w-4 h-4 mr-2"/> Longest Duration</span>
                                <span className="font-bold text-white">{formatDuration(personalRecords.longestDuration)}</span>
                            </div>
                             <div className="flex items-center justify-between">
                                <span className="text-gray-300 flex items-center"><Zap className="w-4 h-4 mr-2"/> Fastest Avg Speed</span>
                 ...
                 <Card>
                    ...
                    <ul className="space-y-3">
                        {runs.slice(0, 3).map(run => (
                            <li key={run.id} className="flex justify-between items-center p-3 bg-gray-800 rounded-md">
                                <div>
                                    <p className="font-semibold">{new Date(run.date).toLocaleDateString()}</p>
                                    <p className="text-sm text-gray-400 truncate max-w-xs">{run.notes || 'No notes'}</p>
                                </div>
                                <div className="text-right flex-shrink-0 ml-4">
                                    <p className="font-bold">{(run.distance_m / 1000).toFixed(2)} km</p>
                                    <p className="text-sm text-gray-400">{run.avg_speed_kmh.toFixed(1)} km/h</p>
                                </div>
                            </li>
                        ))}
